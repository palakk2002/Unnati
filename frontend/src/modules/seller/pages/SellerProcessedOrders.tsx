...
export default function SellerProcessedOrders() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'Processed': return 'bg-[var(--primary-alpha-20)] text-[var(--primary-darker)] border border-blue-200';
  ...
  return (
    <motion.div className="space-y-6" initial="hidden" animate="visible" variants={containerVariants}>
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white p-4 rounded-xl shadow-sm border border-neutral-200">
        <div>
          <h1 className="text-2xl font-bold text-neutral-900 tracking-tight">Processed Orders</h1>
          <p className="text-sm text-neutral-500 mt-1">View and manage orders that are being processed</p>
        </div>
        <div className="flex items-center gap-2 text-sm bg-neutral-50 px-3 py-1.5 rounded-lg border border-neutral-200">
          ...
          <span className="text-neutral-600">Processed Orders</span>
        </div>
      </div>
          ...
          <div className="flex flex-col xl:flex-row gap-4 items-end xl:items-center justify-between">
            ...
            <div className="flex flex-col md:flex-row gap-4 w-full xl:w-auto">
              ...
              <div className="w-full md:w-auto self-end">
                <button onClick={handleExport} className="w-full md:w-auto flex items-center justify-center gap-2 bg-[var(--primary-dark)] hover:bg-[var(--primary-darker)] text-white px-4 py-2 rounded-lg text-sm font-medium transition-all shadow-sm hover:shadow active:scale-95"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4M7 10l5 5 5-5M12 15V3" strokeLinecap="round" strokeLinejoin="round"/></svg><span>Export</span></button>
              </div>
            </div>
          </div>
        </div>
            ...
            <table className="w-full min-w-[800px]">
              ...
              <tbody className="divide-y divide-neutral-100 bg-white">
                {paginatedOrders.length === 0 ? (
                  <tr><td colSpan={6} className="px-6 py-12 text-center"><div className="flex flex-col items-center justify-center"><div className="w-16 h-16 bg-neutral-100 rounded-full flex items-center justify-center mb-4"><svg className="w-8 h-8 text-neutral-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 022-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"></path></svg></div><h3 className="text-lg font-medium text-neutral-900">No processed orders</h3><p className="text-neutral-500 mt-1">Try adjusting your search or filters</p></div></td></tr>
                ) : (
