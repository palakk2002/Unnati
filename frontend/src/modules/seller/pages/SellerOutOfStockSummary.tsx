...
import { getOutOfStockSummary, OutOfStockData } from "../../../services/api/seller/sellerInventoryService";
...
const SellerOutOfStockSummary = () => {
  ...
  const downloadExcel = () => {
    ...
    if (data.length === 0) {
        const headers = ["Name", "Variant", "UOM", "Purchase Value", "MRP", "Selling Price", "Quantity", "Supplier", "Category"];
                      ...
                      <td className="px-3 py-3 bg-red-100">
                        {editMode ? (
                          <input
                            ...
                            className="w-20 px-2 py-1 text-sm border border-gray-200 rounded focus:border-[var(--primary-color)] focus:ring-1 focus:ring-[var(--primary-color)] outline-none font-bold text-red-700"
                          />
                        ) : (
...
export default SellerOutOfStockSummary;
