...
import { getLowStockSummary, LowStockData } from "../../../services/api/seller/sellerInventoryService";
...
const SellerLowStockSummary = () => {
  ...
  const downloadExcel = () => {
    ...
    if (data.length === 0) {
        const headers = ["Name", "Variant", "UOM", "Purchase Value", "MRP", "Selling Price", "Quantity", "Low Stock Qty", "Supplier", "Category"];
                      ...
                      <td className="px-3 py-3 bg-orange-50">
                        {editMode ? (
                          <input
                            ...
                            className="w-20 px-2 py-1 text-sm border border-gray-200 rounded focus:border-[var(--primary-color)] focus:ring-1 focus:ring-[var(--primary-color)] outline-none font-semibold text-orange-700"
                          />
                        ) : (
...
export default SellerLowStockSummary;
