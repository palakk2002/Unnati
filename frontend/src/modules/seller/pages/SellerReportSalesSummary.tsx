...
import { getSalesSummaryReport, SalesSummaryReportData } from "../../../services/api/seller/sellerInventoryService";
import { toast } from "react-hot-toast";

type DateFilterType = 'today' | 'tomorrow' | 'last7days' | 'last30days' | 'alltime' | 'custom';

const SellerReportSalesSummary = () => {
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                ...
                <tr>
                   <td colSpan={15} className="px-6 py-12 text-center text-gray-400 font-medium italic">
                    No sales data found
                  </td>
                </tr>
              ) : (
                data.map((item) => (
                  <tr key={item._id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className="text-[10px] font-bold px-2 py-1 rounded-full bg-[var(--primary-alpha-20)] text-[var(--primary-darker)] capitalize">
                        {item.status}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3 font-bold text-[var(--primary-dark)] text-sm">₹{item.profit.toFixed(2)}</td>
                    <td className="px-4 py-3">
                       <span className={`px-2 py-1 rounded text-[10px] font-black uppercase tracking-widest ${
                          item.mode === 'POS' ? 'bg-[var(--primary-alpha-20)] text-[var(--primary-darker)] border border-blue-200' : 'bg-[var(--primary-alpha-20)] text-[var(--primary-darker)] border border-purple-200'
                        }`}>
                          {item.mode}
                        </span>
                    </td>
...
export default SellerReportSalesSummary;
