...
import { deleteSellerPOSOrder, getSellerPOSOrders, updateOrderStatus, ReportOrder } from "../../../services/api/orderService";
import { useAuth } from "../../../context/AuthContext";
import { toast } from "react-hot-toast";

type DateFilterType = 'today' | 'last7days' | 'last30days' | 'alltime' | 'custom';

const SellerInvoiceReport = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const response = await getSellerPOSOrders(params);
      if (response.success) {
  ...
  const downloadExcel = () => {
    ...
    if (orders.length === 0) {
      const headers = ["Invoice No", "Date", "Customer", "Phone", "Amount", "Payment Method", "Status"];
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 py-4">
          ...
          {showCustomDatePicker && (
            <div className="mt-4 p-4 bg-[var(--primary-alpha-10)]/30 rounded-2xl border border-[var(--primary-alpha-20)] flex gap-4 animate-slide-left">
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-black text-[var(--primary-alpha-50)] uppercase ml-1 tracking-widest">START DATE</label>
                <input type="date" value={customDateRange.start} onChange={(e) => setCustomDateRange({ ...customDateRange, start: e.target.value })} className="px-3 py-1.5 bg-white rounded-lg border border-[var(--primary-alpha-30)] text-sm font-bold text-gray-700 outline-none focus:ring-2 focus:ring-[var(--primary-color)] transition-all shadow-sm" />
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-black text-[var(--primary-alpha-50)] uppercase ml-1 tracking-widest">END DATE</label>
                <input type="date" value={customDateRange.end} onChange={(e) => setCustomDateRange({ ...customDateRange, end: e.target.value })} className="px-3 py-1.5 bg-white rounded-lg border border-[var(--primary-alpha-30)] text-sm font-bold text-gray-700 outline-none focus:ring-2 focus:ring-[var(--primary-color)] transition-all shadow-sm" />
              </div>
            </div>
          )}
          ...
          <div className="mt-4 flex flex-col md:flex-row gap-4">
            <div className="relative flex-1">
               ...
               <input
                ...
                className="w-full pl-11 pr-4 py-2.5 bg-white border border-gray-100 rounded-xl text-sm font-semibold focus:border-[var(--primary-color)] focus:ring-4 focus:ring-[var(--primary-alpha-10)] transition-all outline-none shadow-sm"
                />
            </div>
                  ...
                  orders.map((item: any) => (
                    <tr key={item._id} className="hover:bg-[var(--primary-alpha-10)]/30 transition-colors group">
                      ...
                      <td className="px-6 py-4 text-right">
                         <Link to={`/seller/orders/${item._id}`} className="px-4 py-2 bg-white border border-gray-100 text-[var(--primary-dark)] rounded-xl text-[10px] font-black hover:bg-[var(--primary-dark)] hover:text-white hover:border-[var(--primary-dark)] transition-all active:scale-95 shadow-sm">Details</Link>
                      </td>
                    </tr>
                  ))
                )}
...
export default SellerInvoiceReport;
