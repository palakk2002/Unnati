...
import { getOutOfStockSummary, OutOfStockData } from "../../../services/api/admin/adminInventoryService";
...
const AdminOutOfStockSummary = () => {
  const [data, setData] = useState<OutOfStockData[]>([]);
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await getOutOfStockSummary(params);
      if (response.success) {
      ...
      console.error("Error fetching out of stock summary:", error);
  ...
  const handleCellEdit = (id: string, field: keyof OutOfStockData, value: any) => {
  ...
  const downloadExcel = () => {
    ...
    XLSX.writeFile(workbook, `Out_Of_Stock_Summary_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
  ...
  const downloadPDF = () => {
    ...
    doc.setFontSize(18);
    doc.text('Out of Stock Summary Report', 14, 15);
    doc.setFontSize(10);
    ...
    autoTable(doc, {
      head: [['Product', 'Variant', 'Qty', 'SP', 'Category']],
      body: tableData,
      startY: 28,
      styles: { fontSize: 8 },
    ...
    doc.save(`Out_Of_Stock_Summary_${new Date().toISOString().split('T')[0]}.pdf`);
  };
      ...
      <div className="bg-white border-b border-gray-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Out of Stock Summary</h1>
              <p className="text-sm text-gray-500 mt-1">Items completely out of stock - immediate action required</p>
            </div>
          ...
          <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-1.5 px-1">Category Filter</label>
                <select
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Total Out of Stock Items</p>
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Potential Lost Revenue</p>
            <p className="text-3xl font-black text-orange-600 mt-2">
              ₹{data.reduce((sum, item) => sum + (item.sellingPrice * 10), 0).toLocaleString()}
            </p>
          </div>

          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Categories Affected</p>
        ...
        {/* Critical Alert */}
        <div className="mb-6 bg-gradient-to-r from-red-50 to-red-100 border-l-4 border-red-600 p-4 rounded-lg">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-xs text-red-700 mt-1">These items are completely out of stock. Immediate reorder required!</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gradient-to-r from-red-50 to-red-100 border-b-2 border-red-200">
                  <th className="px-3 py-3 text-left sticky left-0 bg-red-50 z-20 w-10">
                    <input
                      type="checkbox"
                  ...
                  <th className="px-3 py-3 text-left text-xs font-bold text-gray-600 uppercase tracking-wider sticky left-10 bg-red-50 z-10">Name</th>
                      ...
                      <td className="px-3 py-3">
                        {editMode ? (
                          <input
                            ...
                            className="w-20 px-2 py-1 text-sm border border-gray-200 rounded focus:border-pink-500 focus:ring-1 focus:ring-pink-500 outline-none font-semibold text-[var(--primary-dark)]"
                          />
                        ) : (
                      ...
                      <td className="px-3 py-3 bg-red-100">
                        {editMode ? (
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs px-2 py-0.5 bg-red-600 text-white rounded-full font-bold">OUT</span>
                          </div>
                        )}
                      </td>
                      <td className="px-3 py-3">
        ...
        {pagination.pages > 1 && (
          <div className="mt-6 flex flex-wrap items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
            <div className="text-xs font-black text-gray-400 uppercase tracking-widest">
              Showing <span className="text-red-600">{data.length}</span> of <span className="text-red-600">{pagination.total}</span> Records
            </div>
            <div className="flex items-center gap-2">
              <button
                disabled={pagination.page === 1}
                onClick={() => setPagination(prev => ({ ...prev, page: prev.page - 1 }))}
                className="p-2 border border-gray-100 rounded-xl hover:bg-gray-50 disabled:opacity-30 transition-all font-black text-red-600">
                ...
                {[...Array(pagination.pages)].map((_, i) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-9 h-9 flex items-center justify-center rounded-xl text-xs font-black transition-all ${pagination.page === p ? 'bg-red-600 text-white shadow-lg shadow-red-200' : 'hover:bg-red-50 text-gray-500'}`}>
                      {p}
                    </button>
                  );
                })}
...
export default AdminOutOfStockSummary;
